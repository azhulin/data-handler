 ...
 * {@inheritdoc}
 */
$.constraint = Object.assign(Object.assign({}, Data.Handler.constraint), { length: Data.inequalityConstraints(`${$.id}:length`, data => data.length, "Length"), items_unique: [
        `${$.id}:items_unique`,
        (data, { handler }) => {
            const items = new Set();
            for (const [index, item] of data.entries()) {
...
(function ($List) {
    $List.Handler = $;
    $List.id = $.id, $List.constraint = $.constraint, $List.preparer = $.preparer, $List.processor = $.processor;
